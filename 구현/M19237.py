...
# 격자 크기, 상어 개수, 냄새 유지 시간
# 인접한 칸 중 아무 냄새가 없는 칸의 방향으로 잡는다. 그런 칸이 없으면 자신의 냄새가 있는 칸의 방향으로 잡는다.
# => 냄새가 없는 칸으로 이동한다. 없으면 이전 칸으로 돌아간다.
# 동시에 상어가 이동하는데 여러 상어가 한칸에 있을경우
# => 작은 번호 순서대로 상어를 이동시키면서 자신보다 작은 번호 상어가 있으면 이동시키지 않는다. (삭제)
